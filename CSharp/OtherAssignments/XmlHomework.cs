...
namespace CSharp.OtherAssignments
{
    class XmlHomework
    {
      ...
      public  static XElement xmlarticles()
        {
          XElement  Getarticles = new XElement(
            "articles",
      ...
      public static XElement xmlusers()
        {
           XElement Getusers = new XElement(
                "users",
                new XElement("user",
        ...
        static XElement articles = XElement.Load("E:\\新建文件夹\\a.xml");
        static XElement users = XElement.Load("E:\\新建文件夹\\user.xml");
        ...
        {
            var user = from u in users.Descendants("user")
                       join a in xmlarticles().Descendants("article")
                       on u.Element("userId").Value equals a.Element("authorId").Value into ja
                       from result in ja.DefaultIfEmpty()
                       where result == null
                       select u;
            user.Remove();
            users.Save("E:\\新建文件夹\\U.xml");
        }
    }
}
